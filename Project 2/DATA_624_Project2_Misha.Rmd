...
```{r}
cubist_model <- cubist(committees = 10, neighbors = 2) %>%
  set_engine("Cubist") %>%
  fit(PH ~., ph_training)
```
...
Tuning the best-performing Cubist model in terms of the Committee and Neighbors parameters did not seem to improve the performance against our test set, suggesting some overfitting to the training data. We settled on using the original cubist model to move forward with highlighting the most important variables and final predictions. 

### Variable Importance

```{r, echo = FALSE}
x <- cubist_model$fit$usage[0:20,]
x
```

Within our model, it appears that the strongest predictors for subsetting the dataset are:
...
For the linear models then built within these subsets, the strongest predictors are:
